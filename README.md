...
# 03-스탭 - Tasklet
- 지금까지 구현한 방법은 가장 일반적으로 사용하는 스탭 유형인 태스크릿 스텝방식이었다.
이러한 태스크릿 스텝을 만드는 두가지 방법은 MethodInvokingTaskletAdapter 를 이용하거나
HelloWorld.java 에서 구현했듯이 Tasklet 인터페이스의 execute 메서드를 구현하는 방법이 있다.

- 그 밖의 여러 유형의 테스크릿이 있다.
- CallableTaskletAdapter -> CallableTaskletAdapter.java
CallableTaskletAdapter 를 스프링 빈으로 구성한 후에, 해당 어댑터를 스텝 내에 테스크릿으로 등록한다.
CallableTaskletAdapter 는 callable 객체에 대한 단일 의존성만 필요하다.
*CallableTaskletAdapter 는 스레드와 별개의 스레드에서 실행되지만 그렇다고 해서 스탭과 병렬로 실행되는 것은 아니다!
이 스탭이 실행될 때 Callable 객체가 유효한 RepeatStatus 객체를 반환하기 전에는 완료된 것으로 간주되지 않는다.
그러므로 해당 스탭이 완료될 때까지 플로우 내의 다른 스탭은 실행되지 않는다.*

-MethodInvokingTaskletAdapter 를 사용해 일반 POJO 를 스탭으로 활용할 수 있다.
MethodInvokingTaskletAdapter 에서 지정한 메서드가 ExitStatus 를 반환하지 않는 한 결과값으로 ExitStatus.COMPLETED 를 반환한다.
ExitStatus 를 반환하면 메서드가 반환한 값이 태스크릿에서 반환된다. 정적파라미터또한 사용하려면 MethodInvokingTaskletAdapterV2.java 를 참고하자

- SystemCommandTasklet 는 시스템 명령을 실행할 때 사용한다. 지정한 시스템 명령은 비동기로 실행된다.
때문에 타임아웃 설정이 중요하다. interruptOnCancel 속성은 선택사항으로, 잡이 비정상적으로 종료될 때 시스템 프로세스와 관련된 스레드를 강제로 종료할지 정할수 있다.
- SystemCommandTasklet 를 좀더 강력하게 시스템 명령 실행시 영향을 줄 수 있는 여러 파라미터를 구성하고 싶다면 AdvanceCommandTasklet.java 를 참고하자 설명은 책 142p 를 참고하자


# 03-스탭 - Chunk
- CompletionPolicy 인터페이스 구현체를 제공함으로써 청크가 완료되는 시점을 프로그래밍 방식으로 정의할 수 있다.
CompletionPolicy 는 기본적으로 SimpleCompletionPolicy 로 구현되어져 있는데 
이 SimpleCompletionPolicy 는 처리된 아이템 개수를 세는데, 이 개수가 미리 구성해둔 임곗값에 도달하면 청크 완료로 표시한다.
또한 TimeoutTerminationPolicy 를 사용해 타임아웃 값을 구성하면, 청크 내에서 처리 시간이 해당 시간이 넘을 때 안전하게 빠져나갈수 있다(완료되었음을 의미)
일반적으로 TimeoutTerminationPolicy 는 혼자 쓰이지않고, CompletionPolicy 의 일부분으로 사용될 가능성이 높다
이때 CompositeCompletionPolicy 를 사용하면 청크 완료 여부를 결정하는 여러 정책을 함께 구성할 수 있다.
- CompositeCompletionPolicy 는 자신이 포함하고 있는 여러 정책 중 하나라도 청크 완료라고 판단되면 해당 청크가 완료된 것으로 표시한다.
-> ChunkV2.java